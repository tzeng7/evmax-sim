export const meta = {
  name: 'model-improve',
  description: 'Gated model-improvement loop: value-audit finds one actionable sector gap, an agent proposes ONE model-side change, it is measured walk-forward, integrity- and signal-gated, then kept as a PR or reverted. Never edits live model state directly. Safe to run headless in the SHARED main checkout.',
  ...
  phases: [
    ...
    { title: 'Hypothesize', detail: 'rank one novel model-side change (checks ledger)' },
    ...
    { title: 'Dispose', detail: 'keep→PR or revert; append ledger row' },
  ],
}
...
// pick the single most significant gap (most-negative z; calib-bias-only sorts after)
findings.sort((a, b) => (a.z_score ?? 0) - (b.z_score ?? 0))
const target = findings[0]
log(`G1: actionable gap → ${target.sector}/${target.market_type || 'ml'} (${target.gap_type}, z=${target.z_score ?? 'n/a'})`)
...
const spec = await agent(
  ...
  { label: `hypothesize:${target.sector}`, phase: 'Hypothesize', schema: CHANGE_SPEC_SCHEMA, agentType: 'general-purpose' },
)

if (!spec || spec.kind === 'none' || spec.novel === false) {
  log(`G0: no novel change available for ${target.sector} (${spec?.note || 'none proposed'}). Stopping.`)
  return { stopped: 'no_novel_change', sector: target.sector, spec }
}
...
const impl = await agent(
  ...
  { label: `implement:${spec.sector}`, phase: 'Implement', schema: IMPLEMENT_SCHEMA, agentType: 'implementer' },
)

if (!impl || !impl.committed) {
  log(`Implement did not commit for ${spec.sector} (${impl?.aborted_reason || impl?.note || 'no diff'}). Nothing to measure.`)
  return { stopped: 'implement_failed', sector: spec.sector, spec, impl }
}

// ---- N5 · MEASURE (leak-free walk-forward, change already committed on branch) ----
phase('Measure')
const metrics = await agent(
  ...
  { label: `measure:${spec.sector}`, phase: 'Measure', schema: METRICS_SCHEMA, agentType: 'general-purpose' },
)
...
const verdict = metrics?.ran
  ? await agent(
      ...
      { label: `integrity:${spec.sector}`, phase: 'Integrity', schema: VERDICT_SCHEMA, agentType: 'iteration-reviewer' },
    )
...
log(
  `Gates → integrity=${verdict?.accept ? 'ACCEPT' : 'REJECT'} · ` +
...
const dispose = await agent(
    ...
    ? `KEEP this evmax change and open a PR. The change is committed on branch ${impl.branch}.
       ...
       Steps:
         1. Run TARGETED tests only for the touched area, e.g.
            ...
            fall through to the revert path below and record decision "revert/no-change".
       ...
       Return action=kept with the PR url (or action=blocked if targeted tests failed → then also do the revert ledger row).`
  ...
  { label: `dispose:${keep ? 'keep' : 'revert'}:${spec.sector}`, phase: 'Dispose', schema: DISPOSE_SCHEMA, agentType: 'implementer' },
)
...
return {
  ...
  gap: target.gap_type,
  change: spec.params,
  decision: dispose?.action || (keep ? 'kept' : 'reverted'),
  holdout_delta: metrics?.ran ? improvement : null,
  pr_url: dispose?.pr_url || null,
  ledger_written: Boolean(dispose?.ledger_written),
}
