...
| Mode       | What happens |
|------------|--------------|
| `live`     | Scanner produces EVGaps, persists rows with `mode='live'`, sizes Kelly against bankroll. In the shipped YAML this is 7 of 16 categories: `nba`, `nfl`, `ncaab`, `ncaaw`, `soccer`, `tennis`, `wnba`. The other 9 (`worldcup`, `baseball`, `nhl`, `lol`, `cs2`, `ufc`, `nba_props`, `nfl_props`, `baseball_props`) ship in `shadow` — run `evmax categories modes` for the live answer rather than trusting this list. |
...
├── backtest/                # Walk-forward backtest harness behind `evmax backtest run`
├── golf/                    # STANDALONE golf module — NOT in the scan pipeline (golf is an
│                            # N-outcome field; the pipeline is pairwise). Kalshi KXPGATOUR +
│                            # PolyUS golf futures, PGA GraphQL strokes-gained, MC field sim.
│                            # 2026 verdict: FOLD on outrights (no edge vs the live market).
│                            # See docs/golf-module.md
...
├── disk_cache.py            # On-disk TTL cache shared by the clients
├── formatting.py            # Shared CLI table/number formatting helpers
├── provenance.py            # Stamps model_version (git SHA) onto logged rows
└── cli/
...
| NBA Shot Quality | 0.15 | 0.45 | `data/models/shot_quality_state.json` (NBA only; zone FGA + FG% from `nba_api`. NBA override pins it to 0.10 — see the per-sector list below) |
| NBA Matchup | 0.10 | 0.45 | `data/models/matchup_state.json` (NBA only; paint + transition + TOV battle) |
| **WNBA Efficiency** | 0.30 | 0.45 | `data/models/wnba_efficiency_state.json` (seeded by `scripts/seed_wnba_efficiency.py` from 2025 ESPN box scores via Dean Oliver formulas; WNBA-tuned HOME_EDGE_PTS=2.6, SCORE_STDEV=12.5, MIN_GAMES=4 — lowered from 12 when empirical-Bayes shrinkage landed, see the WNBA maintenance section) |
...
| Tennis Form | 0.15 | 0.45 | `data/models/tennis_form_state.json` (recent-match form from Tennis Abstract matchmx via `scripts/seed_tennis_models.py`. The tennis override lifts it to **0.35** — the highest-weighted tennis model — and it is one of the four `REQUIRED_BLEND_MODELS`, so a tennis gap without it is demoted to shadow) |
...
| MLB Props Cache | — | — | `evmax/clients/baseball_props_cache.py` + `evmax/models_ml/baseball_props.py`; season-to-date MLB Stats API rates blended with the Pinnacle anchor via `ev/prop_pricing.py` (see `baseball_props` in the Betting Categories section for stat coverage). Built 2026-06-27 but **not wired into `AgentCoordinator._PROP_SECTORS` until 2026-07-13** — before that fix, `_fetch_props` was never invoked for baseball scans and zero `baseball_props` rows were ever logged despite baseball scanning every 90 minutes. Now fires like NBA/NFL props. |
| **NHL xG** | 0.30 | 0.45 | `data/models/nhl_xg_state.json` (NHL only, shadow, added 2026-05-02; MoneyPuck team 5v5 xGF/60 + xGA/60 → projected goal margin → `Φ(margin / GOAL_STDEV)` with GOAL_STDEV=2.05, HOME_EDGE_GOALS=0.18. Generic Elo kept at 0 — K-factor / home-advantage never calibrated for NHL, this is the NHL half of MODEL-2 still open. Goalie GSAx + special-teams + B2B fatigue tracked in TODO.md as v2/v3. Promote to live once shadow Brier validates against the sharp baseline.) |
...
- **Baseball:** pitcher_v2 0.50 · elo 0.25 · form 0.25 (pitcher renamed pitcher_v2 on 2026-07-19 — see the MLB Pitcher row; poisson is genuinely excluded now — `baseball` was removed from `SUPPORTED_SECTORS` in `poisson_agent.py` on 2026-06-01. Until then the "not listed = never instantiated" assumption was wrong: poisson ran and contributed at its 0.30 class-weight fallback to every baseball blend. See the Poisson row above.)
...
- **NHL:** nhl_xg 0.30 · form 0.15 · elo 0.0 · poisson 0.0 (v1 ships with team 5v5 xG (MoneyPuck) as the dominant non-sharp signal; generic Elo held at 0 pending NHL calibration (MODEL-2 / SECTOR-1); Poisson stays out. `nhl` remains in `shadow` mode — see `data/categories.yaml`.)
...
- **Sharp-only guard (soccer/worldcup, 2026-07-19)**: `MIN_NONSHARP_MODELS` in `ev_gap_agent.py` — an any-N-of floor complementing the tennis all-of gate. Soccer ML (and worldcup ML/advance) live plays need ≥1 genuine model token in `model_sources`; tokens in `_NON_MODEL_TOKENS` (sharp, sharp(capped), injury, late_news, rest, playoff, advance_derived, spread_dist, total_dist, no_side) don't count. Sharp-only rows demote to shadow via the same `full_blend=False` machinery. Motivation: MLS was scanned from 2026-07-09 but never seeded, so 11 sharp-passthrough MLS rows logged as LIVE plays. Sectors without an entry (ufc/lol/cs2/nhl) are unaffected — sharp-dominance there is by design. Soccer totals deliberately excluded (no model prices them). A matching contamination rule dates the pre-guard live sharp-only soccer ML rows out of every gate.
- **Tennis coverage (2026-07-19)**: `merge_ranking_priors` supplements the tour ranking stores with recently-active (≤90d) matchmx players after the weekly TA-leaderboard full-replace — established players churned off the leaderboard (trailing-52wk activity gate) keep firing the surface model's 0.48-confidence ranking-prior fallback instead of collapsing to 0.30 and dropping the blend. The advanced agent's `_lookup` now uses the shared `tennis_common.resolve_player` (dehyphenation, given-name narrowing, same-surname ambiguity → None). NO threshold cuts — the ITF/qualifier tier stays correctly withheld.
- **Why-not diagnostics (2026-07-19)**: `BlendedPrediction.diagnostics` classifies every sector-configured model per event as fired / gated (with the agent's own note, captured at the ensemble 0.45 gate) / missing (returned nothing), persisted as JSON in `ev_predictions.model_diagnostics` (moneyline-family rows only). Inspect via `evmax cleanup shadow show --why`; the promotion board aggregates per-sector top blockers. Separates recoverable coverage gaps (gated-thin / missing-from-state) from correctly-withheld matches without re-running predict_pair.
- **Promotion board (2026-07-19)**: `evmax cleanup shadow board [--days 30] [--sector S] [--staleness-h 3]` + dashboard "Board" tab (`GET /api/promotion-board`, `evmax/agents/cleanup/promotion_board.py`) — one row per (sector, market_type, venue): clean/resolved/logged counts, paired ΔBrier vs sharp, staleness-filtered CLV vs the gate constants, **blend divergence pp** (mean |blended − sharp|; moneyline under 0.5pp = SHARP-PASSTHROUGH — any apparent EV is venue divergence, not model edge), gate checklist and a verdict (SHARP-PASSTHROUGH / COLLECTING n/30 / FAILING-CLV / PROMOTE-READY / LIVE-HEALTHY / LIVE-DEGRADING). The reference snapshot that motivated it (30d ML divergence): wnba 4.82pp vs tennis 0.17 / baseball 0.10 / soccer 0.00.
...
# `com.evmax.watch-listings` (hourly; --log-entries was added to its plist
# on 2026-07-19, so the anchored-entry stream is live).
