...
| Mode       | What happens |
|------------|--------------|
| `live`     | Scanner produces EVGaps, persists rows with `mode='live'`, sizes Kelly against bankroll. The shipped YAML base for `nba`, `nfl`, `ncaab`, `ncaaw`, `soccer`, `tennis`, `wnba`. Of the rest, `worldcup`, `nba_props`, and `baseball_props` ship `disabled` (PR #185, 2026-08-08; `nfl_props` was re-opened to `shadow` on 2026-09-02 by PR #241) and every remaining category ships `shadow`. Read the current split from `evmax categories modes`, never from this table. |
| `shadow`   | Scanner produces EVGaps, persists rows with `mode='shadow'` AND `captured_yes_price = pre-game YES ask`, does NOT touch the bankroll. The validation stage every new sector passes through before promotion (today: `ncaaf`, `ufc`, `nhl`, `baseball`, `lol`, `cs2`, and `nfl_props` for the MODEL-9 validation clock). |
