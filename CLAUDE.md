...
1. **Fetch live prediction-market prices from both venues**: Kalshi via series tickers (e.g. `KXNBAGAME`, `KXEPLGAME`, `KXATPMATCH`, `KXNCAAWBGAME`) and **Polymarket US** via league events (`gateway.polymarket.us /v2/leagues/{slug}/events`, see `POLYMARKET_US_LEAGUE_MAP` — 8 sectors). Markets from both venues merge into one pool; `PredictionMarket.source` / `EVGap.venue` / the `venue` DB column carry the venue through matching → EV → persistence. **Polymarket US is behind a PER-SECTOR venue shadow firewall**: a PolyUS gap logs as `mode='shadow'` with Kelly zeroed UNLESS its sector is cleared, mirroring the Kalshi per-category mode registry. Clearing is via `settings.polymarket_us_sector_live(sector)` — True when the master switch `polymarket_us_live=true` (all sectors) OR the sector is in the `polymarket_us_live_sectors` allowlist (comma-separated, env `POLYMARKET_US_LIVE_SECTORS`). A sector still only goes live if its category mode also resolves to `live` upstream (`get_mode`), so the allowlist refines the venue firewall without overriding a shadow/disabled category — e.g. clearing `wnba` promotes only WNBA **moneyline** on PolyUS because wnba spread/total are `shadow_market_types`. Shipped default: **`polymarket_us_live=true`** (2026-09-18 — master switch flipped ON by owner decision, clearing the firewall for ALL PolyUS sectors at once; the `polymarket_us_live_sectors="wnba"` allowlist is now moot since the master switch short-circuits the per-sector check). This is a DELIBERATE OVERRIDE of the per-sector promotion gate: only `wnba` had genuinely cleared it (2026-08-22 — the only PolyUS market with genuine model divergence from sharp, ~3.9pp, n=82 ML rows / CLV +0.37pp / blend Brier 0.1762 < sharp 0.1801). Every other PolyUS sector is sharp-passthrough (tennis divergence 0.17pp, baseball 0.055pp) or underpowered (nfl/baseball n<30 resolved), so live PolyUS Kelly on those sectors sizes against Kalshi-vs-Poly venue arb rather than model edge. Set `polymarket_us_live=false` to restore the per-sector allowlist discipline. NOTE the master switch does NOT override the dashboard's per-scan `selected_venues` restriction (the bankroll-venue dropdown): a scan scoped to Kalshi still zeroes every PolyUS Kelly, including wnba — selection only ever RESTRICTS. Enforced at three sites: `coordinator.run_cycle` (zero Kelly + drop from live plays), `logger.log_gaps` (demote to `mode='shadow'`), and `web/app._gap_to_dict` (dashboard mode badge). All three route through the GENERIC per-venue dispatch `settings.venue_sector_live(venue, sector)` (Kalshi always clear; every other venue gated per-sector; `polymarket_us_sector_live` is the PolyUS delegate) — so any non-Kalshi venue is shadow-firewalled by the same machinery. `polymarket_us_enabled=false` kills the fetch entirely. **P2P exchanges Novig + ProphetX are wired as first-class venues** (`MarketSource.novig`/`.prophetx`, `VENUE_LABELS`, fee models in `evmax/fees.py`, and `novig_live`/`novig_live_sectors` / `prophetx_live`/`prophetx_live_sectors` firewall settings, default fully-up) EXCEPT the market-data CLIENT: both APIs require an authenticated account (docs.prophetx.co Market Data API, docs.novig.com — no public unauth endpoint like PolyUS's), so `novig_enabled`/`prophetx_enabled` stay `false` and no scan emits these sources until a client lands and gates its fetch on the flag. **Fee models are load-bearing and structurally different: Novig** is the SAME quadratic entry fee as Kalshi/Poly (taker θ=0.03, cap $0.0075/contract at p=0.5; maker earns a rebate at half the taker rate) — cheaper than both at every price. **ProphetX** is NOT an entry fee — a 2% commission on NET WINNINGS charged at settlement on winning contracts only (VIP 1.5%), the betting-exchange model. It is expressed for EV as the breakeven-probability SHIFT it induces (`prophetx_fee_prob` = `q − p` where `q = 1/(1 + (1/p−1)(1−r))`), so `effective_price` and arb price every venue uniformly; realized P&L (`bet_pnl`) applies the winners-only profit commission (its true mechanism). At p=0.5 the ProphetX drag is ~0.5pp of true prob vs Kalshi's 1.75pp — the exchange advantage, and the reason maker/exchange execution is a larger EV lever than any model edge. Match/EV dedup keys are venue-aware (same game on both venues = two independent books). **Close capture covers both venues:** `cleanup watch-closes` snapshots near-tip asks for Kalshi AND PolyUS bets into `archived_kalshi_markets` — PolyUS snapshots are keyed by the full venue-prefixed market id (`polymarket_us:slug[:side]`, no schema change; lowercase slugs can't collide with Kalshi tickers), and `backfill_clv` resolves the archive ticker venue-aware via `resolver.close_lookup_ticker` (`kalshi:` prefix stripped, `polymarket_us:` kept, `:no` suffix stripped and the close read from the archived NO ask). **Live price re-verify at pick time covers PolyUS:** `evmax agents pick`/`verify` refresh PolyUS asks over the gateway (`get_markets(sector, fresh=True)` — cache bypassed) via `fetch_polymarket_live_asks` in `cli/commands/agents.py`, so a PolyUS row gates/sizes at its current ask, not the stale scan ask. PolyUS quotes BOTH sides, so a NO-side row reads `no_price` directly — no `1-yes` spread fudge. A PolyUS-gateway failure demotes only PolyUS rows to their scan price; it never clears the run's `live` flag. Flipping `polymarket_us_live=true` still waits on the shadow gates (n≥30 resolved/sector, CLV≥0 on the venue's own book with adequate close-capture coverage).
...
├── models_ml/
│   ├── spread_distribution.py  # Normal CDF for spread market cover probabilities (NFL: delegates to spread_pmf)
│   └── spread_pmf.py        # NFL key-number margin PMF (artifact data/models/nfl_margin_pmf.json, refit by scripts/fit_nfl_margin_pmf.py)
├── agents/
...
| Sharp (Pinnacle) | 0.85 (CLI/config default) | always | auto-tuned in `data/model_config.json`. **Effective model share is (1−sw)² ≈ 2.25% at a 50/50 line, not 15%**: `_flb_correct` re-blends the already-blended prob toward sharp (non-NBA sectors), so share = (1−sw)·(1−esw)·(1−1.5(s−½)²). Kept deliberately — the 2026-09-22 replay found the ~30%-share (0.40) rows significantly worse than sharp (ΔBrier +2.97/1000, z 2.9; NCAAF +5.6, worldcup ROI −47%) and no sector with evidence for more model weight. NBA skips FLB: its share is (1−0.70)=0.30. |
...
- **Spread/total dedup keys the LINE** (`matching/engine.py::match_all`): a single Pinnacle `::spread` record (the main line) is exact-matched by EVERY Kalshi/PolyUS alternate-spread rung of the same game/side, so the dedup key `(event_id, market_type, yes_side, venue, line)` MUST include the line — without it all but the first-listed rung collapse (the 2026-09-06 NFL scan dropped 874 of 1137 matched markets, one rung/game). Each surviving rung is still priced off that one Pinnacle record via `SpreadDistributionModel` (`spread_dist`, gated to ±1σ from the main line in `ev_gap_agent`; NFL uses the key-number PMF below, token `spread_pmf`), so keeping every rung is the intended coverage, not a mispricing. Ties in the date-delta tie-break now prefer higher match confidence. (Pinnacle's own `isAlternate` spread ladder is deliberately NOT parsed — the sign/convention reconciliation is a mispricing risk on shadow-only sectors; `spread_dist` off the main line already covers the tradeable band.) **Alt-spread ladder (2026-09-11, INERT):** `SPREAD_LADDER_ENABLED` (in `spread_distribution.py`, default **False** = the behaviour just described, byte-for-byte) reverses that decision behind a flag: the client emits one `SharpOdds` per alternate spread rung (`is_alternate=True`, event_id `::spread::<line>`, main line keeps the bare `::spread`), the matcher's `_find_nearest_spread` prefers the rung whose \|line\| matches the market's (side-agnostic; `align_yes_side` orients downstream), and `ev_gap` takes that rung's devigged cover prob DIRECTLY (token `sharp_ladder`), skipping `SpreadDistributionModel`. Rationale: the normal CDF misprices alt rungs vs the book's own rung — for NFL by 2–5pp on rungs that cross 3/7 (0.73pp vs 2.90pp MAE on 17 archived Pinnacle ladders, see the PMF bullet below). (The older example here, "−16.5 off a −3 main: CDF 0.167 vs book 0.077", most likely compared the favorite-side CDF with an UNDERDOG-side rung: the favorite −16.5 fair is ≈0.16 — 2003–25 empirical 0.159 — and ≈0.07–0.08 is the underdog winning by 17+. Note both "fav −16.5" and "dog −16.5" rungs parse to the same `::spread::-16.5` event_id.) Phase 3 also adds `SPREAD_MAX_SIGMA` (default 1.0 = today's gate) + `_SPREAD_MAX_ABS_LINE` (empty) to tighten the CDF gap-filler for rungs the ladder doesn't cover. Golden fixtures pin the sign/convention across NFL/NBA/NCAAB (`tests/test_spread_alt_ladder.py`). Validate + flip per sector with `scripts/backtest_spread_ladder_replay.py` (deep-tail Brier) then CLV (`cleanup shadow clv <sector> -m spread --sources-token sharp_ladder`) — never on Brier alone; NBA/NCAAB/NCAAW spread is LIVE, so the default-off flag is the only thing between here and a pricing change.
- **NFL key-number spread PMF (2026-09-22)**: `SpreadDistributionModel.predict` prices NFL alt rungs with a discrete favorite-margin PMF (`models_ml/spread_pmf.py::MarginPMF`, artifact `data/models/nfl_margin_pmf.json`) instead of the normal CDF (σ=14): NFL margins carry ~15% on ±3 and ~9% on ±7, which the normal spreads evenly. Discrete normal × signed key-number multipliers with a spread-bucket interaction (bucket keyed on the MAIN LINE) and spread-dependent width; μ is anchored so `P(F > a | F ≠ a)` equals Pinnacle's devigged main-line cover prob exactly (push-conditional on whole numbers). Sign conventions are `predict`'s. Its gate uses the TRUE distance on the favorite-margin axis, so deep "underdog wins by X" rungs that the normal path's folded `abs(abs(target)−abs(main))` gate lets through go unpriced (that folded gate is untouched). Evidence (fit nflverse 2003–18, holdout 2019–25 scored once, 1,960 games / 55,924 rungs): ΔBrier −1.62/1000 vs the juice-anchored normal (z −4.95, game-clustered), all on rungs crossing 3/7 (−2.40, z −5.7); 7/7 walk-forward seasons better; MAE vs Pinnacle's own archived alt ladder 0.73pp vs 2.90pp (16/17 games closer); on our 1,107 resolved NFL spread shadow rows, rows the PMF also rates ≥2% EV earned +1.41pp Kalshi CLV (game-declustered z 8.8) vs −0.36pp for rows it rates negative-EV. Switch: `_PMF_SECTORS = {"nfl"}` in `spread_distribution.py` (remove to revert); missing/invalid artifact → normal fallback (logged once). Token `spread_pmf` (non-model, in every `_NON_MODEL_TOKENS` mirror); a contamination rule dates the old `spread_dist` NFL spread rows out of every gate, so the NFL spread CLV sample restarts on PMF pricing (old sample: `shadow metrics --include-contaminated`). NFL spreads stay `shadow_market_types`; promote per side on CLV — `cleanup shadow clv nfl -m spread --sources-token spread_pmf --side lay|take`, declustered by game — never on Brier. Refit each offseason with `python scripts/fit_nfl_margin_pmf.py` (frozen hyperparameters; prints holdout + walk-forward; `--dry-run` writes nothing). The research sketch keyed the bucket on the anchored μ, whose root sits on the μ=3/7 jump for common lines (−3 @ p≈0.5) and missed the devig by up to 2pp; keying on the main line is Brier-equivalent (inner 2015–18 +0.04/1000, holdout −0.02) and exact.
- **Pinnacle parallelism**: all `(sport_key × market_type)` combinations fetched simultaneously
- **Pinnacle game path uses the bulk straight index** (`esports_pinnacle.get_odds`, 2026-09-06): fetches `/sports/{sport_id}/markets/straight` ONCE and serves each matchup's ML/spread/total from it (`_fetch_bulk_straight_index` + `_fetch_matchup_odds(..., markets_override=)`), falling back to the per-matchup `/matchups/{id}/markets/related/straight` only for a bulk miss. The per-matchup endpoint intermittently 403s (US geo-block) and a single 403 silently deleted a whole game's ladder; the bulk endpoint is the same one the props path already uses to dodge that block. Cuts NFL from 17→2 HTTP calls. `market_counts` in `pinnacle_scan_done` now keys off `spread_line`/`total_line` (SharpOdds has no `market_type` field — the old `getattr` labeled every record `moneyline`).
...
- **Sharp weight has ONE source of truth — `data/model_config.json`** (`sharp_weight` global + `sharp_weight_by_sector`; soccer per-league via the tier YAML). `AgentCoordinator(sharp_weight=None)` reads it, so the CLI, dashboard, Discord-triggered and portfolio scans blend identically; `log_gaps` stamps each row's BASE blend weight (`logger.stamped_sharp_weight`: per-sector entry, else the scan global; soccer = league tier), which the pick-time re-blend uses — the base weight, not the true sharp share after the disagreement ramp + FLB re-blend (≈1−(1−sw)² ≈ 0.98 at 0.85), so the re-blend is an approximation except for NBA (no FLB). **Before 2026-09-22** the coordinator defaulted to 0.40 and the dashboard never passed a weight, so every dashboard scan blended sectors without a per-sector entry (nfl/ncaaf/wnba/ncaab/ncaaw/nhl/worldcup) at 0.40 while the CLI used 0.85, and every row was stamped 0.85 regardless — dashboard ML rows diverge 2.7–3.6pp from sharp vs 0.2–0.4pp for CLI rows (`scan_sector_stats.source` join, ledger exists from 2026-09-06 only). The WNBA "genuine model divergence" behind its promotions came largely from these 0.40 rows, but it is NOT evidence for a lower WNBA weight: 97% of WNBA rows also carry the (pre-08-08 full-size) injury adjustment, so a model-weight effect is not identifiable from them, and every sector where models carried ~30% share was significantly worse than sharp (2026-09-22 replay). Set per-sector weights only from clean evidence in `sharp_weight_by_sector`, never via a surface default.
- **CLV measurement rules (2026-09-22)**: (1) NO-side rows (`:no`) are scored ASK-to-ASK — the close is the archived NO ask (`archiver.get_kalshi_close_price(side="no")`; `watch-closes` captures it via `KalshiClient.get_market_quotes_batch` from 2026-09-22), never `1 − yes_ask` (the NO bid, which charged every NO row the spread). Older YES-only captures stored a SYNTHESIZED `no_price = 1 − yes` (detectable: `yes + no == 1` exactly, impossible for a two-sided book); NO rows skip those snapshots, so a NO row with only synthesized closes gets NO CLV rather than a bid-valued one. (2) CLV is FORWARD-only from each row's own entry (`resolver.clv_not_before`: fill, else first-log `logged_at`) — at/after-tip entries get no close (UFC's +2.38pp was mostly backward-looking in-play rows); UNPLACED Kalshi rows also never take a close from inside (T-30, tip) (`resolver.clv_near_tip_ok` — tennis in-play prints), placed and PolyUS rows may. (3) The CLV lenses drop cancelled voids and `minutes_to_tipoff=0` entries but KEEP `stale_reverted` voids (dropping them would bias CLV up). (4) The CLV gate's sample size counts independent GAMES (`shadow.game_key`), not rows/rungs — the board shows `(Ng)`. (5) A 0/1 devigged close is never a price: `get_closing_line_aligned` rejects it and `backfill_outcome_closes` heals stored zeros (164 totals outcomes had `pinnacle_close_prob=0.0` via alt-rung `::total::<line>` ids). Stored historical values change only on an explicit `evmax cleanup backfill-clv --recompute [--dry-run]`; a real recompute first writes `data/predictions_backup_before_clv_recompute_<ts>.db`.
...
- **YES-side alignment is a dedicated stage** — `evmax/matching/alignment.py::align_yes_side` is the ONLY place that decides which sharp outcome a venue's YES contract prices (`YesOutcome` a/b/draw/over/under); the EV agent's step 1 consumes the `Alignment` and never compares strings itself. Rules, strongest first, each required to hit EXACTLY ONE side: canonical equality → unique token-subset (surname ⊆ full name) → closed-world venue-team step (resolve the YES label against the market's OWN two teams by equality/subset/ticker-code prefix+acronym, then map that team onto the sharp side) → price-distance fallback for `PRICE_FALLBACK_SECTORS` (lol/cs2 bare ticker codes) only. Ambiguous or unresolvable ⇒ `None` ⇒ the market is NOT priced (counted per sector as `yes_alignment_failed_summary`; a wrong side is a booked loss, a dropped market is a missed play). **Never reintroduce fuzzy scoring or "default to outcome A" here**: the 2026-09-05 incident — `rapidfuzz.token_set_ratio("western michigan","michigan")==100` with outcome A tested first — priced the Western Michigan / Florida Atlantic / Washington State YES contracts at the favorite's 94–97% (+974…2145% EV), and the same rule scores Man City/Man United and Rams/Chargers ≥80. `alignment_looks_suspect` logs `yes_alignment_suspect` when the OTHER side sits on the ask. Golden containment table in `tests/test_alignment.py`. **Outcome resolution and close alignment use the same rules (2026-09-22):** `_match_espn`/`_match_bo3` in `resolver.py` had the identical tie-to-team-A `token_set_ratio` bug (4 NCAAF underdog rows graded WON on favorite wins), and `yes_aligned_close_prob` had it as substring matching (a live tennis "maria" ⊂ "Maria Sakkari" close). They now go through `alignment.resolve_team` plus a strict slug→ESPN bijection (`_assign_pair`: alias-canonical > token-subset > fuzzy, where two KNOWN canonicals never match, and exactly one assignment must win), with ambiguity ⇒ unresolved (`resolver_ambiguous_event` / `resolver_yes_side_unresolved`). Repair: `scripts/repair_containment_outcomes.py` (dry-run default); goldens in `tests/test_resolver_name_containment.py`.
- **Soccer is ONE sector with a per-LEAGUE dimension, not eight sectors** (2026-09-05): UCL/UEL pair clubs from different domestic leagues, so Elo/Poisson/xG need one shared club pool, and the registry/resolvers/alias maps/promotion gates are sector-keyed (splitting would also fragment every n≥30 gate — EPL took five months to reach 42 resolved). What differs by league is POLICY and MEASUREMENT: [`evmax/sectors/soccer_leagues.py`](evmax/sectors/soccer_leagues.py) is the single source of league identity (canonical keys `epl laliga bundesliga seriea ligue1 ucl uel mls ligamx jleague eredivisie brasileirao championship` — `SOCCER_LEAGUES`; Kalshi series prefix → league, PolyUS league slug → league). `PredictionMarket.league` is stamped by both venue parsers, rides `EVGap.league` into the `ev_predictions.league` column (migration + idempotent backfill on every `get_connection`: Kalshi rows from the ticker series prefix, PolyUS rows from the slug's SECOND dash token — `atc-mls-…`/`tsc-epl-…`; only that token is trusted because a team code can equal a league slug, `sea` = Seattle vs Serie A), and is the filter behind `cleanup shadow show/metrics/clv/board --league` and the `cleanup shadow clv-leagues soccer` segmentation (dashboard `/api/promotion-board?league=`). `data/soccer_league_tiers.yaml` is keyed by league (`sharp_weight_for_league` / `sharp_weight_for_market`) — this FIXED a silent bug where every PolyUS soccer market (no Kalshi ticker) fell to the 0.40 MLS default — and a tier may carry its own `disagreement_ramp`, threaded coordinator → `EnsembleModelAgent` as `disagreement_by_event` → `_disagreement_sharp_weight(params=)`. **Ship-state is blend-neutral:** the top tier's ramp equals the sector ramp (0.04/0.10/1.00) and MLS carries none (inherits it). The known problem this exposes: the sector ramp was calibrated on pooled top-5 data and pushes MLS to 100% sharp on any ≥10pt disagreement, erasing the 0.60 model share on exactly the games where a secondary-league edge could show (live MLS divergence 0.8pp at sharp_weight 0.40). **MLS-only walk-forward (2026-09-05, `scripts/mls_disagreement_ramp.py`, docs/mls-disagreement-ramp-eval.md) REFUTED the ramp thesis:** the close is the ceiling on MLS too — flat 0.40 with no ramp is +13.8/1000 3-way Brier worse than sharp-only on the 2026 holdout (z 2.9), the inherited ramp is within noise of sharp, and no swept ramp beats it outside noise, so MLS keeps inheriting. The 0.40 MLS base was unsupported on close-Brier and survived only because the ramp capped it; **2026-09-22 it moved to 0.85** (and `default_sharp_weight` 0.40→0.85) on `clv-leagues` evidence — MLS 59 games, +0.65pp, 47% positive fails the %pos gate — plus the blend-weight replay finding ~0.36-model-share rows significantly worse than sharp (ΔBrier +2.97/1000, z 2.9). A league earns a weight below 0.85 only on its own CLV bucket (CLV, not Brier — the tennis lesson). **Per-league MODEL parameters were also walk-forward REJECTED the same day** (`scripts/backtest_soccer_league_params.py`, docs/soccer-league-params-eval.md): per-league Poisson `league_avg` is −3.9/1000 in-sample (z −3.0) but −0.16/1000 (z −0.08) on holdout and nil in the blend; train-swept per-league Elo HFA flips sign by league on holdout (Bundesliga best at 30 in-sample, +7.8/1000 worse out of sample) and is nil in the blend. Poisson keeps one soccer-wide `league_avg`, Elo keeps HOME_ADVANTAGE_ELO 60. The league dimension earns its keep on measurement (CLV per league), not on model parameters. **Five more leagues wired 2026-09-05 (Liga MX `KXLIGAMXGAME`/Pinnacle 2242/ESPN `mex.1`/PolyUS `lmx`, J League `KXJLEAGUEGAME`/2157/`jpn.1`, Eredivisie `KXEREDIVISIEGAME`/1928/`ned.1`, Brasileirão `KXBRASILEIROGAME`/1834/`bra.1`/`bra`, Championship `KXEFLCHAMPIONSHIPGAME`/1977/`eng.2`/`eflch`)** — chosen from a venue probe (Liga MX out-trades Bundesliga and Ligue 1 on Kalshi; second divisions, UECL, Nations League and domestic cups had ~zero open markets). They sit in the 0.85 top tier and on the tier YAML's **`shadow_leagues` list — the LEAGUE-level shadow**: mode is per sector, so a league newly wired into the live soccer sector would otherwise be live on its first scan. A shadow-listed league still scans/matches/prices/LOGS (mode='shadow', feeds `clv-leagues`) but is Kelly-zeroed and dropped from the live play list at the same three sites as the PolyUS venue firewall (`coordinator.run_cycle` league_gaps_shadowed, `logger.log_gaps` prediction_demoted_shadow_league, `web/playlist`). Lift with `evmax cleanup shadow promote-league <league>` (refuses unless the league's own CLV bucket clears the `clv` gate; `--force` overrides). Adding a league = SECTOR_SERIES_MAP + KALSHI_SERIES_LEAGUE/SOCCER_LEAGUES/LEAGUE_DISPLAY + Pinnacle `SECTOR_SPORT_LEAGUES`/`SOCCER_DRAW_LEAGUES` + PolyUS slug maps + resolver `ESPN_SOCCER_LEAGUES` + both seed scripts' league lists + tier YAML + aliases; `tests/test_soccer_extra_leagues.py` pins all of it. **Gotchas from the wiring:** the soccer normalizer does NOT strip accents (ESPN `América`/`León`/`Grêmio` need explicit aliases); ESPN names Liga MX's Santos Laguna plain `Santos`, which is Brazil's Santos canonical — `ESPN_DISPLAY_NAME_OVERRIDES` in `soccer_leagues.py` renames it at every ESPN extraction site (resolver + both seeds) so the clubs can't merge into one Elo entity; and both seed scripts 403'd on every league until their `evmax-seeder/1.0` User-Agent was swapped for the resolver's `_ESPN_HTTP_UA` (the xG seed now aborts WITHOUT writing when it fetched nothing — it had persisted an emptied club store).
...
# adds calibration bias, promotion-gate clearances and the per-sector devig-method
# recommendation (WEEKLY_CHECKS = calibration/gates/devig). ONE alert at worst severity.
...
evmax cleanup shadow clv-leagues soccer [-m moneyline] [--venue kalshi]  # soccer-only: resolved CLV segmented per league (one bucket per key in `SOCCER_LEAGUES` — epl/laliga/bundesliga/seriea/ligue1/ucl/uel/mls/ligamx/jleague/eredivisie/brasileirao/championship — plus 'unknown' for pre-column PolyUS rows) against the same gate as `clv`. The per-league policies (tier sharp_weight, disagreement ramp — data/soccer_league_tiers.yaml) must be judged here, per league, before either is changed. `--league L` is also accepted by shadow show/metrics/clv/board.
evmax cleanup shadow promote-league ligamx   # lift a league off soccer's `shadow_leagues` (data/soccer_league_tiers.yaml) once ITS OWN clv-leagues bucket clears the clv gate; --force overrides
evmax cleanup shadow clv-prices nfl [-m moneyline] [--venue kalshi] [--side lay]  # ANY sector: segment resolved rows by OUR-side ENTRY-PRICE bucket (0-10c … 90c+; key = resolver.clv_entry_price — placed fill else scan ask, the exact CLV anchor; `:no` rows already store their own side's ask so no flip). Per bucket it reports BOTH lenses: calibration (realized win% vs mean blend/sharp, Δpp = blend − realized; > 0 = the blend over-states the bucket) AND Kalshi CLV vs the same gate as `clv`, plus a z. The favorite–longshot readout: the EV% gate mechanically favors cheap contracts (+2pp edge = +40% EV at 5c vs +2.2% at 90c) and the blend leans toward underdogs early-season (own data 2026-09-19: blend − realized +3.4pp at 10-20c / +3.8pp at 35-50c, ≤0 on favorites; entry→close CLV is flat-positive in every bucket, so the miscalibration is in OUTCOMES, not CLV). Judge any gate/calibration change here per bucket first. `--price-bucket 35-50` is also accepted by `clv` and `board` (+ `GET /api/promotion-board?price_bucket=`) as a filter. Helper: evmax/agents/cleanup/price_buckets.py.
...
# ML is live; spread/total are disabled_market_types (anchored-entry owns them) — watch their promotion metrics
...
- **WNBA moneyline is `live`** (`data/categories.yaml`). Spread + total are `disabled_market_types`, so the SCANNER persists neither (thinner Kalshi totals volume; `wnba_possession_sim`'s totals output is newer to live) — their shadow rows come from the `watch-listings --log-entries` anchored-entry trigger, and they promote via the anchored-entry CLV lens, not the moneyline gate.
