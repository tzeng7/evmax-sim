...
- **Spread/total dedup keys the LINE** (`matching/engine.py::match_all`): a single Pinnacle `::spread` record (the main line) is exact-matched by EVERY Kalshi/PolyUS alternate-spread rung of the same game/side, so the dedup key `(event_id, market_type, yes_side, venue, line)` MUST include the line — without it all but the first-listed rung collapse (the 2026-09-06 NFL scan dropped 874 of 1137 matched markets, one rung/game). Each surviving rung is still priced off that one Pinnacle record via `SpreadDistributionModel` (`spread_dist`, gated to ±1σ from the main line in `ev_gap_agent`), so keeping every rung is the intended coverage, not a mispricing. Ties in the date-delta tie-break now prefer higher match confidence. (Pinnacle's own `isAlternate` spread ladder is deliberately NOT parsed — the sign/convention reconciliation is a mispricing risk on shadow-only sectors; `spread_dist` off the main line already covers the tradeable band.) **Alt-spread ladder (2026-09-11, INERT):** `SPREAD_LADDER_SECTORS` (in `spread_distribution.py`, a PER-SECTOR allowlist, default **empty** = the behaviour just described, byte-for-byte) reverses that decision: for a sector in the allowlist the client emits one `SharpOdds` per alternate spread rung (`is_alternate=True`, event_id `::spread::<line>`, main line keeps the bare `::spread`), the matcher's `_find_nearest_spread` prefers the rung whose \|line\| matches the market's (side-agnostic; `align_yes_side` orients downstream), and `ev_gap` takes that rung's devigged cover prob DIRECTLY (token `sharp_ladder`), skipping `SpreadDistributionModel`. The single gate is `spread_ladder_enabled(sector)`, called at all three sites. Rationale: the normal CDF overstates the deep tail 2–3× vs the book's own rung (measured NFL −16.5 off a −3 main: CDF 0.167 vs book 0.077), which is the +20–70% phantom-EV nickel-spread mechanism. **Low-scoring sectors (baseball/nhl/soccer) are HARD-excluded** by `spread_ladder_enabled` regardless of the allowlist — the ladder path skips `predict()` and would otherwise bypass the ±1.5 `_LOW_SCORING_MAX_ABS_LINE` cap that killed the 2-for-15 baseball −4.5 alt run-line. Phase 3 also adds `SPREAD_MAX_SIGMA` (default 1.0 = today's gate) + `_SPREAD_MAX_ABS_LINE` (empty) to tighten the CDF gap-filler for rungs the ladder doesn't cover. Golden fixtures pin the sign/convention across NFL/NBA/NCAAB + the low-scoring exclusion (`tests/test_spread_alt_ladder.py`). Add a point-margin sector (nfl/nba/ncaab/ncaaw/ncaaf/wnba) to the allowlist only after ITS OWN `scripts/backtest_spread_ladder_replay.py` (deep-tail Brier) + CLV (`cleanup shadow clv <sector> -m spread --sources-token sharp_ladder`) clears — never on Brier alone; NBA/NCAAB/NCAAW spread is LIVE, so the empty allowlist is the only thing between here and a pricing change.
