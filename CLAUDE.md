...
- **Sharp weight has ONE source of truth — `data/model_config.json`** (`sharp_weight` global + `sharp_weight_by_sector`; soccer per-league via the tier YAML). `AgentCoordinator(sharp_weight=None)` reads it, so the CLI, dashboard, Discord-triggered and portfolio scans blend identically; `log_gaps` stamps each row's EFFECTIVE weight (`logger.stamped_sharp_weight`), which the pick-time re-blend uses. **Before 2026-09-22** the coordinator defaulted to 0.40 and the dashboard never passed a weight, so every dashboard scan blended sectors without a per-sector entry (nfl/ncaaf/wnba/ncaab/ncaaw/nhl/worldcup) at 0.40 while the CLI used 0.85, and every row was stamped 0.85 regardless — dashboard ML rows diverge 2.7–3.6pp from sharp vs 0.2–0.4pp for CLI rows (`scan_sector_stats.source` join, ledger exists from 2026-09-06 only). The WNBA "genuine model divergence" behind its promotions was mostly these 0.40 rows (resolved ML: div≥1.5pp 196 games Kalshi CLV +0.44pp t 2.25 vs div<1.5pp 42 games 0.00pp) — set per-sector weights from evidence in `sharp_weight_by_sector`, never via a surface default.
