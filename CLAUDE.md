...
- **Persistence is surface-independent; display floors are not (2026-09-20).** `CycleResult.persistable_gaps(range_start, range_end)` (coordinator) is THE row set both `agents scan` and the dashboard `/api/scan` log to `ev_predictions`: every agent-floor game-market gap (props excluded, partial-blend included → shadow) whose Kalshi game-day falls in the window (`gap_in_persist_window`, which applies `persist_window`'s weekly-sector horizon). Before this the CLI logged only rows that also cleared its `min_prob`/EV display floor while the web logged everything, so every CLV/Brier/price-bucket sample silently depended on which surface ran the scan. `min_prob` / `min_ev` (`ev/calculator.py::passes_play_floor`) are a PLAY-surface policy — scan display, `verify`, `pick`, `prune-stale` — re-applied against the LIVE price; they never decide persistence. The old `tiered_min_ev` longshot ramp was removed: it was dead code at every call site (the `min_prob` floor ran first, so its extra term was always 0). Longshot admission lives in the agent-side probability-space floor (`dual_ev(edge_min_pp=)`), not in an EV% ramp.
...
- **Probability-space admission floor (favorite–longshot guard, 2026-09-20, INERT)**: `dual_ev(..., edge_min_pp=)` in `evmax/ev/calculator.py` AND-s a `(blended − fee-inclusive price)·100 ≥ edge_min_pp` floor with the EV floor, per execution mode (taker/maker), and exposes `taker_edge_pp`/`maker_edge_pp`/`ev_passes`. Resolved per sector by `ev_gap_agent.resolve_edge_min_pp` (code map `EDGE_MIN_PP_BY_SECTOR` > `settings.edge_min_pp` > 0.0); applied at all four gate sites (YES, NO-spread, under, props) with a per-cycle `edge_pp_floor_dropped_summary` count. **Ships OFF** (map empty, setting 0.0) — the identity is unit-tested across a price/venue grid, so admission is byte-identical today. Why: the EV% gate is ~18× easier to clear at 5c than 90c for the same pp edge (`+2pp` = +40% EV vs +2.2%) while Kelly sizes on `edge/(1−price)` — the gate admits exactly the longshot rows the sizer distrusts, and own resolved rows show the blend +3.4/+3.8pp above realized at 10-20c/35-50c. Flip a sector ONLY on the replay (`scripts/backtest_sizing.py --edge-min-pp 0,1,2,3 --modes shadow --include-unsized --sector <s>` — `make_gated_policy` zeroes below-floor rows; shadow rows need `--include-unsized` or the loader returns 0 rows) plus `cleanup shadow clv-prices <s>` — never on Brier. Not a replacement for `matching/alignment.py` (an alignment error stamps the wrong side's prob, so its pp edge is huge and passes) nor for the sizing quarantine (which needs blended far from sharp; an aligned-wrong row has blended ≈ sharp).
