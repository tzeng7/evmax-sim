...
| Tennis Ranking Trend | 0.10 | 0.45 | `data/models/tennis_ranking_trend_state.json` (reworked 2026-06-10: logit = 0.40·Δlog-rank **absolute** + capped(0.15·12-week **log-space** momentum, ±0.40); anchored at `market.event_date` with a 6-week staleness guard — eval Brier 0.2530 → 0.2249, acc 49.8% → 64.2% on 2025+2026 walk-forward, params swept on 2024 only (`scripts/experiment_ranking_trend.py`). The old raw-spots 0.5-centered nudge was coin-flip noise. **Reseed weekly** via `scripts/reseed_tennis_rankings.py` (Sackmann full depth — it rolls `{tour}_rankings_current.csv` each season, so the old seed path froze the state at 2024-12-30 for 18 months; the reseed merges the `_20s` decade file + current file) **then** `scripts/refresh_tennis_rankings_espn.py` (ESPN keyless top-150 both tours, current week, idempotent — order matters: the reseed full-replaces the store). Scheduled task `weekly-tennis-rankings-refresh` runs both every Tuesday 07:00. **DEGRADED 2026-06-27:** the Sackmann reseed step now 404s (the `tennis_atp`/`tennis_wta` repos went offline), so `ranking_trend` is maintained by the **ESPN refresher alone** — shallower (top-150, current week only), but enough to keep the 6-week staleness guard satisfied. A full-depth replacement source is still TODO.) |
