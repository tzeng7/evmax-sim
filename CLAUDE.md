...
| Mode       | What happens |
|------------|--------------|
| `live`     | Scanner produces EVGaps, persists rows with `mode='live'`, sizes Kelly against bankroll. Each category's base mode lives in the YAML — don't assume live; roughly half the catalog sits in shadow at any given time. |
...
**Outcome resolution** is specified per-category via the `resolver` field. The shipped values are `espn_scoreboard` (NBA/NFL/NCAAB/NCAAW/soccer/worldcup/baseball/nhl/wnba — worldcup reads ESPN `fifa.world`), `espn_boxscore` (NBA/NFL/MLB props), `bo3gg` (LoL/CS2), `kalshi_settlement` (tennis), and `none` (no auto-resolution wired yet). Do not maintain a separate "resolution table" in docs — this field is authoritative.
...
| NBA Props Cache | — | — | `data/nba_props_cache.json` (daily L15 game logs — **diagnostics only since 2026-05-10** (edb3d7b): sample size + minutes volatility. The L15 prob model (per-36 + opponent adj + isotonic) was ripped out; prop P(over) now comes from Pinnacle anchor pricing in `evmax/ev/prop_pricing.py`, fed to `_evaluate_prop_pair` via `sharp.true_prob_over`) |
| NFL Props Cache (QB only v1) | — | — | reuses `data/backtest/nfl_props/*.parquet`; `evmax/clients/nfl_props_cache.py` is a data/diagnostics layer only since 2026-05-10 (edb3d7b) — the L15-style prob model (weighted mean + opponent adj + Poisson TD helpers) was removed with the NBA one; pricing goes through the same anchor path. Stage 5 live Kelly gated on 2026 shadow validation (MODEL-9) |
...
flowchart TD
    ...
    subgraph FETCH[1. Data fetch · parallel fan-out]
        ...
        I[InjuryReportAgent<br/>ESPN public API<br/>NBA/NFL/NCAAB/NCAAW/WNBA/soccer]
