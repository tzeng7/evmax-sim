...
- **CLV measurement rules (2026-09-22)**: (1) NO-side rows (`:no`) are scored ASK-to-ASK — the close is the archived NO ask (`archiver.get_kalshi_close_price(side="no")`; `watch-closes` now captures it via `KalshiClient.get_market_quotes_batch`), never `1 − yes_ask` (the NO bid, which charged every NO row the spread: NFL spread NO −0.19→+0.55pp, baseball total NO −0.58→+0.18, WNBA spread NO −1.76→−1.00 on recompute). (2) CLV is FORWARD-only from each row's own entry (`resolver.clv_not_before`: fill, else first-log `logged_at`) — at/after-tip entries get no close (UFC's +2.38pp was mostly backward-looking in-play rows → −0.75). (3) The CLV lenses drop cancelled voids and `minutes_to_tipoff=0` entries but KEEP `stale_reverted` voids (dropping them would bias CLV up). (4) The CLV gate's sample size counts independent GAMES (`shadow.game_key`), not rows/rungs — the board shows `(Ng)`. (5) A 0/1 devigged close is never a price: `get_closing_line_aligned` rejects it and `backfill_outcome_closes` heals stored zeros (164 totals outcomes had `pinnacle_close_prob=0.0` via alt-rung `::total::<line>` ids). Stored historical values change only on an explicit `evmax cleanup backfill-clv --recompute [--dry-run]` (dry run 2026-09-22: 1,335 of 4,843 rows move).
