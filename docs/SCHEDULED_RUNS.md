...
| `daily-close-lines-capture` | 08:04 | `evmax cleanup close-lines` — **effectively a no-op today, see the warning below**; real close capture is `watch-closes` under launchd |
...
> **⚠ `daily-close-lines-capture` cannot capture anything (verified 2026-08-03).** `evmax
> cleanup close-lines` selects `FROM ev_outcomes WHERE outcome IS NULL AND
> pinnacle_close_prob IS NULL`, but the only code path that INSERTs a row into `ev_outcomes` is
> `resolver._write_outcome` (`evmax/agents/cleanup/resolver.py:1115`), and it always supplies a
> concrete `outcome` — so no row is ever pending. (Other paths only UPDATE `pinnacle_close_prob`
> in place; none creates a row or nulls `outcome`.) Measured: 0 of 3741 `ev_outcomes` rows have
> `outcome IS NULL`; the task prints "No unresolved markets without closing lines" every day.
> Genuine closes come from `com.evmax.watch-closes` + `cleanup backfill-clv`. Needs a decision:
> repoint the query at unresolved `ev_predictions`, or retire the task. The task is left ENABLED
> and untouched (it is harmless) pending that call.
