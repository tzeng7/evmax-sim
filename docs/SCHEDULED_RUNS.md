...
- **2026-09-14** (weekly drift audit) the `com.evmax.integrity` row's `--weekly` description was
  completed: the sweep gained a third weekly check, `devig`, in PR #280
  (`WEEKLY_CHECKS = ("calibration", "gates", "devig")`), which surfaces the one-tap
  `evmax cleanup devig promote <sector>` when a non-power devig method clears its gate. Task
  inventory re-verified against the live registry — all 19 registered tasks match this doc at the
  documented cadence, and no documented job lacks a backing task, so no task rows changed. Full
  report: `docs/drift-audits/2026-09-14.md`.
