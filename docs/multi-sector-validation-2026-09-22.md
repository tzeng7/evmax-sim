...
| 6 | **NFL alt spreads priced by a normal CDF (σ 14) ignore key numbers.** | Key-number margin PMF: holdout 2019–25 ΔBrier −1.62/1000 (z −4.95), 7/7 walk-forward seasons, all gain on rungs crossing 3/7; MAE vs Pinnacle's own ladder 0.73pp vs 2.90pp; on our rows CLV +1.41pp where the PMF agrees vs −0.36pp where it doesn't (in-sample selection — supporting evidence only); per the research replay ~38% (419/1,107) of logged NFL spread "+EV" rows are negative-EV under the PMF. | #317 |
...
| 8 | **NCAAF training data corrupt** (ESPN scoreboard-delta `score_points`; mislabeled `drive.team`). Reseed blocked two weeks. | 720 illegal deltas / 353 games (2021–26); ~2,100 possession-flipped plays / 800 games. Validated feed (legal-amount deltas, possession from `start.team`, monotone scoreboard): v2 pooled Brier 0.1867→0.1859 (flat), open→close slope +0.084→+0.103 (t 13.0); games crediting more points than the final 2,028→74. | #314 |
| 9 | **Two sectors silently dead.** UFC: Kalshi short titles "{Name} wins" → 0 matched since 08-23. NHL: no alias map → 0 matched EVER; season opens 2026-09-29. Integrity never alerted (zero baseline). | UFC live replay 0/50 → 28/50 matched (all 28 fights Pinnacle has posted; YES side correct 28/28); NHL archive replay 0/236 ML → 182/236 with the map. LoL/CS2 have the same short-title break (0 matched; follow-up task). | #325, #321 |
...
6. #318 MLS 0.85. 7. #321 NHL (before 2026-09-29; regenerate — never hand-merge — its state JSON if it conflicts). CLAUDE.md conflicts are expected across #315/#316/#317/#318/#321/#323 (adjacent single-line paragraphs): keep BOTH sides' edits — never take one side of a multi-KB paragraph wholesale. 8. #325 UFC parser + baseline-free zero-match integrity streak. 9. #323 model name resolution (then the NCAAF Elo/form rebuild it lists).
