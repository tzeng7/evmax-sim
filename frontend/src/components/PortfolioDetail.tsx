...
function BetRow({ bet, showResult }: { bet: PortfolioBet; showResult: boolean }) {
  ...
  return (
    <tr>
      ...
      <td className="num">{probToCents(bet.kalshi_yes_price || 0)} <span className="muted">({probToAmerican(bet.kalshi_yes_price || 0)})</span></td>
      <td className="num">{probToCents(bet.blended_true_prob || 0)} <span className="muted">({probToAmerican(bet.blended_true_prob || 0)})</span></td>
